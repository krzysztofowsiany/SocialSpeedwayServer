...
function updateSyncData(playerid, data) {
	try {
		db.queryNoResults('UPDATE synchronize SET achievements=$2 WHERE playerid=$1;', [playerid, data]);
	}
	catch(e)
	{
		console.error(e);
...
exports.getData = function (data, gameSocket) {	
	try {
		db.queryResults(				 
				'SELECT p.strength, p.agility, p.speed, p.endurance, s.achievements FROM playerskills p JOIN synchronize s ON s.playerid=p.playerid WHERE p.playerid=$1;'
				,[data['playerID']],
				function (results)	{				
					gameSocket.emit('syncAchievementsResultData', results[0]);					
				}
		);
	}
	catch(e)	
